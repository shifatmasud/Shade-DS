...
-   **`SEP 07, 2026`** · **Onpress-Gated Touch Drag-to-Select**: Refined touch text selection in the Terminal / TUI (`Terminal.tsx`). Implemented a 250ms press-and-hold (`onpress`) gesture recognition gate with jitter suppression. Quick swipes and vertical flicks maintain natural viewport panning, while a deliberate press-and-hold activates selection mode (with subtle haptic feedback) before dragging to expand the text range.
